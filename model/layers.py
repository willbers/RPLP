...
class SpGraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, entity_embeddings_mapping, edge, edge_embed, edge_list_nhop, edge_embed_nhop, edge_type):
        N = input.size()[0]
        ...
        if self.concat:
            projected_head = entity_embeddings_mapping[edge_type, edge[0, :], :]
            projected_tail = entity_embeddings_mapping[edge_type, edge[1, :], :]
            edge_h = torch.cat((projected_head, projected_tail, edge_embed[:, :]), dim=1).t()
